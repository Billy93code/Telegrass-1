# PowerShell script to fix meta tags and canonical URLs in all Hebrew city pages
# This fixes og:url, twitter:url, canonical href, and structured data URLs
...
Get-ChildItem -Path $citiesPath -Filter "*-כיוונים.html" | ForEach-Object {
    ...
    try {
        ...
        # Fix each English URL pattern in meta tags and canonical links
        foreach ($englishUrl in $urlMappings.Keys) {
            $hebrewUrl = $urlMappings[$englishUrl]
            
            # Replace in various URL contexts
            $content = $content -replace "cities/$englishUrl", "cities/$hebrewUrl"
            $content = $content -replace "$englishUrl", "$hebrewUrl"
        }
        ...
        if ($content -ne $originalContent) {
            ...
            $updatedFiles++
            Write-Host "✓ Updated meta URLs in: $fileName" -ForegroundColor Green
        } else {
